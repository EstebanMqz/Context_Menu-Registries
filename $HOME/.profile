...
# If the following commands are created/enabled with the .sh script, the .bashrc / .profile would load infinitely (load itself), nevertheless for their existance in other than $HOME, do.
# [ -f ~/.profile ] && . ~/.profile
# [ -f ~/.bashrc ] && . ~/.bashrc
